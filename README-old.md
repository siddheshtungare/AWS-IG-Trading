...
![The bot](./Images/Workflow.png)

## IG Trading bot

A CFD trading bot built to trade using the IG brokerage APIs. The Bot is built as a 3-step AWS Step function and has been implemented using the AWS SAM CLI.

**Sub-processes**

**1. Login and initial checks**

This initial step checks the access to IG APIs by creating a new session. This uses your IG Account credentials - username, password, API keys. The user credentials are stored as environment variables in the Lambda function.

This function also reads the config file for the bot. The config file contains the list of symbols, their marketids(needed by the IG APIs) and some strategy specific parameters.

![config file](/Images/config.png)

The config file contains the list of symbols, their marketids(needed by the IG APIs) and some strategy specific parameters.

The next step of the bot uses the `Map state` step of the AWS Step Functions.

**2. Data gathering and strategy evaluation**

_Write a bit about the second step_

**3. Trade execution**

_Write a bit about the third step_

### References

- [IG API Documentation](https://labs.ig.com/gettingstarted)
...
```bash
IG-Trading-1$ sam build --use-container
```
...
```bash
IG-Trading-1$ sam logs -n StockCheckerFunction --stack-name "ig-trading-1" --tail
```
...
```bash
IG-Trading-1$ pip install -r tests/requirements.txt --user
# unit test
IG-Trading-1$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
IG-Trading-1$ AWS_SAM_STACK_NAME="ig-trading-1" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "ig-trading-1"
```
