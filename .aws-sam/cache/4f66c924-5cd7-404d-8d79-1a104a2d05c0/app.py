                ...
                # If there are existing positions in the same direction: 
                    ...
                    # 1. Check if any of those positions are same as the new position we are trying to create 
                        ...
                        for position in df_positions_db.itertuples():
                            print(f"Comparing existing position price points {position.price_points} with new signal price points {trade_params['price_points']}")
                            if position.price_points == trade_params['price_points']: 
                                print(f"Found matching position in DynamoDB with DealId {position.DealId}")
                    ...
                    # 2. If they are different, create a new position
                            ...
                            print("Duplicate position detected - same direction and price points. Skipping position creation.")
...
def create_db(position_details): 
    ...
    current_time = dt.datetime.now(tz=pytz.timezone('Australia/Sydney')).strftime(format='%Y-%m-%dT%H:%M:%S')
...
def read_db(): 
    ...
    # If no items were returned, return an empty DataFrame with required columns
    if not response['Items']:
        return pd.DataFrame(columns=['Id', 'Timestamp', 'DealId', 'Epic', 'Direction', 'Status', 'price_points'])

    df_data = pd.DataFrame(response['Items'])
    
    # Print raw data for debugging
    print(f"Raw data from DynamoDB: {response['Items']}")
    
    # Only convert Timestamp if it exists
    ...
    print(f"Columns in DataFrame: {df_data.columns.tolist()}")
    print(f"Number of records: {len(df_data)}")
    print(f"Unique DealIds in DB: {df_data['DealId'].unique().tolist()}")
